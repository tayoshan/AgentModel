...
def get_data(file_path):
    ...
    pos = np.zeros((numSites, 3))
    for site in range(numSites):
        pos[site] = site, float(data[site][4]), float(data[site][3])
...
class agentH:
    def __init__(self, win, site, dists, pos, color):
        ...
        self.loc = gp.Point(pos[self.currentSite][1], pos[self.currentSite][2]) #Assigns the lat/long coordinates for plotting
    ...
    def updateLoc(self, destImp, choiceSiteNum, color):
        ...
        self.startx, self.starty = self.pos[self.currentSite][1], self.pos[self.currentSite][2] #Assign x1 and y1 from the coordinate of the old site
        self.loc = gp.Point(self.pos[self.currentSite][1], self.pos[self.currentSite][2])   #Change location to the coordinate of the new site
        self.endx, self.endy = self.pos[choiceSiteNum][1], self.pos[choiceSiteNum][2] #Assign x2 and y2 from the coordinates of the new site that the agent visited
...
class agentS:
    def __init__(self, win, siteNum, pos, colorBook):
        #Graphics parameters
        self.win = win  #set window to draw settlement in
        self.x = pos[siteNum][1]    #Store x and y coords of this settlement
        self.y = pos[siteNum][2]
        ...
        self.importance = 1 #Set starting importance to the default of 1
        self.visitors = 1   #Visitor count starts at 1 at beginning of simulation
    ...
    def updateAttbs(self, currentSite, originImp, home):
...
class context:
    def __init__(self, numSites, numAgents, pos, dists, colorBook):
        ...
        for i in range(numSites):
            self.agentsS.append(agentS(self.win, i, pos, colorBook[i]))
    ...
    def move(self, human, resourceBenefits, communicationDifficulty):
        ...
        for settlement in self.agentsS: #For each settlement in the list of all settlements
            ...
            if self.agentsH[human].distance(settlementNum) > 0 and self.agentsH[human].distance(settlementNum) < 60:    #If distance from this agent to each settlement is not 0 or more than the limited vision distance
    ...
    def output(self):
        out = file("importance.csv", "w")
        numList = []
        imp = []
        for each in self.agentsS:
            numList.append(each.siteNum)
            imp.append(each.importance)
            out.writelines("%s\t%s\n" %  (each.siteNum, each.importance, ))
        out.close()
        return zip(numList, imp)

    def run(self, iterations, numAgents, pos, numSites):
        maxVisitors = 1
        allAgents = np.arange(0, numAgents*numSites)  #Number of settlements times then number of agents that start at each settlement
        for i in range(iterations):
            ...
            for human in allAgents:  #For each agent
                ...
                if attractiveness.size != 0:    #If there is at least 1 site that meets the minimum critera then choose the most attractive one
                    ...
                    self.agentsH[human].updateAttbs(choice[0], choice[1])
                    counter +=1
                    #tm.sleep(.1)
...
def go():
    ...
    numSites, dists, pos = get_data(r'C:\Users\ImAwesome\My Documents\RadiationModel\SiteData.csv')
...
#Update the chosen settlement based on the importance of the site the human started at and the current color of the human agent
